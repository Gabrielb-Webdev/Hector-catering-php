...
$sql = "SELECT titulo_img_carousel, carousel_detail_img, detail_titulo, detail_producto, icon_eventos FROM detalle_eventos WHERE evento_id = $eventoId";
...
if ($result->num_rows > 0) {
    ...
    // Construir un objeto JSON que incluya el título, el título del detalle, el producto, las imágenes del carrusel y los iconos de eventos
    $detallesEvento = array(
        "titulo" => $evento["titulo_img_carousel"],
        "detailTitulo" => $evento["detail_titulo"],
        "detailProducto" => $evento["detail_producto"],
        "imagenes" => $imagenesCarrusel,
        "iconosEventos" => json_decode($evento["icon_eventos"], true)
    );
